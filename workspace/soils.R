...
data = read_sf('/Volumes/Transcend/ngen/GLHYMPS/GLHYMPS.gdb',
               "Final_GLHYMPS_Polygon",
               wkt_filter = sf::st_as_text(bb2))

d2 = st_transform(data, st_crs(template))

glhymps = fasterize::fasterize(d2, template, background = NA,
                               field = 'Permeability_permafrost') %>%
  raster::mask(st_transform(conus, st_crs(template)),
               filename = "/Volumes/Transcend/ngen/GLHYMPS/permeability_permafrost.tif")
